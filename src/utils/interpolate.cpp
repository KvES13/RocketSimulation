...
bool interpolate::Interp1d::need_sort(const std::vector<double>& x) {
    double slope;
...
void interpolate::Interp1d::ascending_order_sort(std::vector<double>& x, std::vector<double>& y) {
...
interpolate::Interp1d::Interp1d(const Interp1d& from) {
    x_src = from.x_src;
    y_src = from.y_src;
...
interpolate::Interp1d& interpolate::Interp1d::operator=(const Interp1d& from) {
    if (this != &from) {
        x_src = from.x_src;
        y_src = from.y_src;
        fill_value = from.fill_value;
...
double interpolate::Interp1d::operator()(const double x) {
...
double interpolate::Linear1D::polate(const double& x, const std::vector<double>& x_src, const std::vector<double>& y_src, const int& fill_value) {
...
interpolate::CubicSpline1D::CubicSpline1D(const std::vector<double>& x, const std::vector<double>& y) {
    SolveSplineCoefficient(x, y);
};


interpolate::CubicSpline1D::~CubicSpline1D() {
    delete[] a_spline;
    delete[] b_spline;
    delete[] c_spline;
    delete[] d_spline;
...
interpolate::CubicSpline1D::CubicSpline1D(const CubicSpline1D& from) {
...
interpolate::CubicSpline1D& interpolate::CubicSpline1D::operator=(const CubicSpline1D& from) {
    if (this != &from) {
        array_size = from.array_size;
        a_spline = new double[from.array_size];
        for (int i=0; i < from.array_size; ++i) {
...
void interpolate::CubicSpline1D::SolveSplineCoefficient(const std::vector<double>& x, const std::vector<double>& y) {
...
double interpolate::CubicSpline1D::polate(const double& x, const std::vector<double>& x_src, const std::vector<double>& y_src, const int& fill_value) {
    int index_target = 0;
    if (x < x_src[0]) {  // 左外挿
        switch (fill_value) {
        case 0:  // zero
